...
import { BoundUnitMemo, Memo, MemoDeclaration, unitMemo } from '../caches';
...
export function byUnit<V>() {
  return <Row extends RowShape>(def: DerivedValuesDef<Row, V>): UnitCacheDeclaration<Row, V> => ({ kind: 'byUnit', def });
}

/** Whether a {@linkcode Partitions.cache | cache} block entry is a {@linkcode byUnit} cache. */
export function isUnitCacheDeclaration(decl: unknown): decl is UnitCacheDeclaration<RowShape, unknown> {
  return typeof decl === 'object' && decl !== null && (decl as { kind?: unknown }).kind === 'byUnit';
}
...
export function derivedValueMemo<V>(max: number): MemoDeclaration {
  return unitMemo<V | undefined>()({ max, by: ['scope'] }) as MemoDeclaration;
}
...
export function createDerivedValues<Row extends RowShape, Key, V>(
  ...
  const warnOnThrash = (count: number): void => {
    ...
    console.warn(
      `[${store}_store] the '${name}' cache was asked for ${count} units but holds ${def.max}, so this read ` +
