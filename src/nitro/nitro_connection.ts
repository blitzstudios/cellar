...
export function openNitroMemoryFallback(dbName: string, opts?: Pick<NitroConnectionOptions, 'shredInJs'>): SqliteConnection {
  const inMemory = openNitroInMemory(dbName, opts);
...
/**
 * Opens `:memory:<dbName>` and returns it if SQLite reports its main database has no file, which is what nitro 1.1.5
 * and later open for that name. An older nitro opens a file called `:memory:<dbName>` instead; that file is deleted,
 * and this returns `undefined`.
 */
function openNitroInMemory(dbName: string, opts?: Pick<NitroConnectionOptions, 'shredInJs'>): SqliteConnection | undefined {
  ...
  try {
    const conn = openNitroConnection(name, { shredInJs: opts?.shredInJs });
    const [main] = readRows<{ file: string | null }>(conn, "SELECT file FROM pragma_database_list WHERE name = 'main';");
    if (main?.file === '') return conn;
  } catch {
    /* no usable database under this name; the scratch file below is the fallback */
  }
  discardNitroDatabase(name);
  return undefined;
}
