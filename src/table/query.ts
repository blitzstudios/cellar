 ...
 */
export function digestColumns<Row extends RowShape>(schema: RowTableSchema<Row>): Array<keyof Row & string> {
 ...
 */
export function digestRow<Row extends RowShape>(row: Row, columns: readonly (keyof Row & string)[]): string {
  let digest = '';
