...
export function createVersionedSourceCache<V>(maxEntries: number, diagnostics: MemoDiagnostics): VersionedSourceCache<V> {
  const watch = __DEV__ ? createMemoWatch(diagnostics, maxEntries) : undefined;
  const lru = createBoundedLru<{ version: number; source: unknown; value: V }>(maxEntries, watch?.onEvict);
  return {
    peek(key, version) {
      const hit = lru.get(key);
      const found = hit && hit.version === version ? hit : undefined;
    ...
    put(key, version, source, build) {
      ...
      if (watch) {
        if (reused) watch.noteHit();
        else watch.noteMiss(key, !hit);
      }
      const value = reused ? hit.value : build();
      lru.set(key, { version, source, value });
      return value;
    },
  };
}
 ...
 */
export function byVersion<V>(spec: { max: number; keyedBy: string; isEqual?: (prev: V, next: V) => boolean }): MemoDecl<VersionedCache<V>> {
  return { keyedBy: spec.keyedBy, build: (diagnostics) => createVersionedCache<V>(spec.max, spec.isEqual, diagnostics) };
}
 ...
 */
export function bySource<V>(spec: { max: number; keyedBy: string }): MemoDecl<VersionedSourceCache<V>> {
  return { keyedBy: spec.keyedBy, build: (diagnostics) => createVersionedSourceCache<V>(spec.max, diagnostics) };
}
 ...
 */
export function declareMemos<D extends Record<string, MemoDecl<unknown>>>(
  store: string,
  decls: D,
): { [K in keyof D]: D[K] extends MemoDecl<infer C> ? C : never } {
  const out = {} as { [K in keyof D]: D[K] extends MemoDecl<infer C> ? C : never };
  for (const memo of Object.keys(decls) as (keyof D & string)[]) {
    out[memo] = decls[memo].build({ name: `${store}.${memo}`, keyedBy: decls[memo].keyedBy }) as never;
  }
  return out;
}
