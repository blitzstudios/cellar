...
exports.byPartition = byPartition;
  ...
  return spec => ({
    ...
    bind: (store, diagnostics) => {
      ...
      return {
        for: key => {
          const prefix = (0, _args_key.cacheKeyOf)(store.parts(key));
          const entryKey = (entityId, parts) => keyer(`${prefix}${_args_key.KEY_SEP}${entityId}`, parts);
          return {
            read: (entityId, ...args) => {
              const {
                parts,
            ...
            readMany: (entityIds, ...args) => {
              ...
              const built = (0, _read_coverage.covered)(() => build(missing.map(entry => entry.entityId)));
              for (const {
                entityId,
                at,
                version
