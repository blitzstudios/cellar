...
exports.createVersionedSourceCache = createVersionedSourceCache;
...
function createVersionedSourceCache(maxEntries, diagnostics) {
...
function identityOf(part) {
  ...
  const identity = (0, _args_key.stableKey)(part);
...
function sourceOf(source) {
  return Array.isArray(source) ? source.join(_args_key.KEY_SEP) : source;
}
...
function byVersion() {
  return spec => ({
    ...
    bind: (store, diagnostics) => {
      ...
      return {
        for: key => {
          const prefix = (0, _args_key.cacheKey)(...store.parts(key));
