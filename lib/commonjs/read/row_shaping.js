...
exports.indexRowsBy = indexRowsBy;
exports.mapRows = mapRows;
exports.orderedByIds = orderedByIds;
/**
 ...
 */
function mapRows(rows, toVm, empty) {
  const out = [];
...
function orderedByIds(rows, idColumn, ids, toVm, empty) {
  const byId = new Map();
...
function indexRowsBy(rows, column, toVm, empty) {
  const out = {};
  let found = false;
  for (const row of rows) {
    const vm = toVm(row);
