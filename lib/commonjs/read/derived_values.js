...
exports.isEntityCacheDeclaration = isEntityCacheDeclaration;
...
function isEntityCacheDeclaration(decl) {
...
function createDerivedValues(ctx, def) {
  ...
  const scopeOf = (key, extra) => !extra || isSingleRow(key) || !Object.keys(extra).length ? WHOLE_ENTITY : (0, _args_key.stableKey)(extra);
  ...
  const overFilter = (key, extra) => {
    ...
    const members = (0, _read_coverage.covered)(() => table.entityIdsWhere(scope));
