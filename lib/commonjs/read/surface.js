...
function createReadSurface(kernel) {
  ...
  const versionOf = partitions => {
    let sum = 0;
    for (const parts of partitions) if ((0, _version_atom.isLive)(parts)) sum += kernel.version.get(parts);
    return sum;
  };
  ...
  const hasAny = entries => entries.some(entry => (0, _version_atom.isLive)(entry.parts) && hasOne(entry.key, entry.parts));
  ...
  const makeValueCache = def => (0, _caches.createVersionedCache)(def.getCacheMax ?? 256, def.isEqual);
  function defineRead(def) {
    ...
    const gatesFor = (args, parts, vary, wanted) => readGates(def, args, wanted && (0, _version_atom.isLive)(parts), vary);
    ...
    function getValue(args) {
      ...
      if (!(0, _version_atom.isLive)(parts)) return def.empty;
    ...
    function useValue(args, options) {
      ...
      const data = kernel.version.useSelect(parts, gates.read, [argsKey], () => hasOne(key, parts) ? cached(args, key, parts, argsKey) : def.empty, def.isEqual ?? Object.is, def.empty);
      ...
      return useReadTail(data, gates.read, () => (0, _version_atom.isLive)(parts) && hasOne(key, parts), prime, doRefetch);
    }
    return {
      getValue,
      useValue,
  ...
  function manyRead(def, resolve, select, noneNamed) {
    ...
    const gatesFor = (args, partitions, vary, wanted) => readGates(def, args, wanted && partitions.some(_version_atom.isLive), vary);
    ...
    function getValue(args) {
      ...
      if (gates.prime) for (const entry of entries) if ((0, _version_atom.isLive)(entry.parts)) primeIfCold(entry.key, entry.parts);
    ...
    function useValue(args, options) {
      ...
      const data = kernel.version.useSelectMany(partitions, gates.read, [argsKey], () => hasAny(entries) ? cached(args, named, partitions, argsKey) : def.empty, def.isEqual ?? Object.is, def.empty);
      const doRefetch = (0, _react.useCallback)(() => {
