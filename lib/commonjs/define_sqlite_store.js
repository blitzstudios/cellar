...
function defineSqliteStore(config) {
  ...
  const buildOnSqlite = (conn, recovery) => {
    const guarded = (0, _connection.guardedConnection)(conn, (error, op) => onSqliteFailure(error, op, recovery), (error, op) => (0, _telemetry.reportStoreDegradation)({
    ...
    return config.build((0, _sqlite.createSqliteRowTable)(config.schema, guarded, config.nativeShredSpec), version, config.sqliteCapabilities?.(guarded) ?? NO_CAPS);
  };
