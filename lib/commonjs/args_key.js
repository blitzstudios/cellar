...
Object.defineProperty(exports, "cacheKeyOf", {
  enumerable: true,
  get: function () {
    return _key.cacheKeyOf;
  }
});
...
function warnOnceIfNotPlainData(value) {
  ...
  console.warn(`[cellar] keyed by a ${name}, which is not plain data: only own enumerable properties count towards a key, ` + `so two different ${name}s would key alike and share one cache entry. Key by the values you mean instead.`);
}
...
function freezeKeyPart(value) {
  ...
  if (proto !== Object.prototype && proto !== null) return;
  Object.freeze(value);
  for (const entry of Object.values(value)) freezeKeyPart(entry);
}
...
function partitionsKey(partitions) {
  return partitions.map(_key.cacheKeyOf).join(GROUP_SEP);
}
...
function varyKey(parts, vary) {
  if (!vary.length) return (0, _key.cacheKeyOf)(parts);
  ...
  const joined = new Array(parts.length + vary.length);
  ...
  return (0, _key.cacheKeyOf)(joined);
}
