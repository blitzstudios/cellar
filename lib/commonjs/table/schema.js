...
function schemaFingerprint(schema, nativeShredSpec) {
  const canonical = (0, _args_key.cacheKey)(`r${schema.rebuildVersion ?? 0}`, schema.table, columnsCanonical(schema), ...structureCanonical(schema, shredFingerprint(nativeShredSpec)));
...
function schemaStructureStamp(schema, nativeShredSpec) {
  const canonical = (0, _args_key.cacheKey)(`r${schema.rebuildVersion ?? 0}`, schema.table, ...structureCanonical(schema, shredStructureFingerprint(nativeShredSpec)));
 ...
 * duplicates.
 */
function createTableSql(schema, temporary = false) {
 ...
 */
function createMetaTableSql(meta, temporary = false) {
