...
exports.addColumnSql = void 0;
exports.addedColumns = addedColumns;
...
exports.readApplicationId = readApplicationId;
exports.readLiveSchema = readLiveSchema;
...
exports.schemaStructureStamp = schemaStructureStamp;
...
function columnsCanonical(schema) {
  return (0, _types.columnNames)(schema).map(column => `${column}:${schema.columns[column].type}:${schema.columns[column].notNull ? 1 : 0}`).join('|');
}

/**
 * Everything but the columns: the parts of a declaration that describe what the rows already on disk *mean*, rather
 * than how many fields they have. Both stamps read these from the same expressions, so the pair cannot drift.
 */
function structureCanonical(schema, nativeShredSpec) {
  ...
  return [`pk(${schema.primaryKey.join(',')})`, indexes.join('|'), meta, shredFingerprint(nativeShredSpec)];
}
...
function schemaFingerprint(schema, nativeShredSpec) {
  const canonical = (0, _args_key.cacheKey)(`r${schema.rebuildVersion ?? 0}`, schema.table, columnsCanonical(schema), ...structureCanonical(schema, nativeShredSpec));
 ...
 */
function schemaStructureStamp(schema, nativeShredSpec) {
  const canonical = (0, _args_key.cacheKey)(`r${schema.rebuildVersion ?? 0}`, schema.table, ...structureCanonical(schema, nativeShredSpec));
 ...
 */
function addedColumns(schema, live) {
  const declared = (0, _types.columnNames)(schema);
...
const addColumnSql = (schema, column) => `ALTER TABLE ${schema.table} ADD COLUMN ${column} ${schema.columns[column].type};`;
...
function readApplicationId(conn) {
  const rows = (0, _connection.readRows)(conn, 'PRAGMA application_id;');
  return rows[0]?.application_id ?? 0;
}

/** Everything `init` plans from, in the three PRAGMA reads it takes to find it. */
function readLiveSchema(conn, table) {
  const rows = (0, _connection.readRows)(conn, `PRAGMA table_info(${table});`);
  return {
