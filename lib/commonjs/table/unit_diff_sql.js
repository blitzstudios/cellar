...
exports.unitDiffSql = unitDiffSql;
 ...
 */
function stageNames(table, fingerprint, path) {
...
function unitDiffSql(schema, names, maxBinds) {
  const {
    table,
    primaryKey,
    unit
  ...
  function replaceKeyed(where, id) {
    const filter = (0, _query.whereClause)(where);
    ...
    [`INSERT INTO ${changes} (write_id, unit) SELECT DISTINCT ?, m.${unit} FROM ${table} m${inPartition} ` + `NOT EXISTS (SELECT 1 FROM ${stage} s WHERE ${on});`, [id, ...filter.params]], [`DELETE FROM ${table}${inPartition} ${unit} IN ${changedUnits};`, [...filter.params, id]], [`${insertVerb} INTO ${table} (${colList}) SELECT ${colList} FROM ${stage} WHERE ${unit} IN ${changedUnits};`, [id]]];
  }
  function replaceKeyless(where, id) {
    ...
    return [[counted, [id]], [`INSERT INTO ${changes} (write_id, unit) SELECT ?, ${unit} FROM (${held} EXCEPT ${staged}) ` + `UNION SELECT ?, ${unit} FROM (${staged} EXCEPT ${held});`, [id, ...filter.params, id, ...filter.params]], [`DELETE FROM ${table}${inPartition} ${unit} IN ${changedUnits};`, [...filter.params, id]], [`INSERT INTO ${table} (${colList}) SELECT ${colList} FROM ${stage} WHERE ${unit} IN ${changedUnits};`, [id]]];
  }
  function merge(id) {
    const moved = `FROM ${stage} s LEFT JOIN ${table} m ON ${on} WHERE ${differs}`;
    return [[counted, [id]], [`INSERT INTO ${changes} (write_id, unit) SELECT DISTINCT ?, s.${unit} ${moved};`, [id]],
...
//# sourceMappingURL=unit_diff_sql.js.map