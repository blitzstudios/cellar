...
var _unit_diff = require("./unit_diff.js");
...
function createMemoryRowTable(schema) {
  if (__DEV__) (0, _query.assertUnitColumn)(schema);
  const hasPk = schema.primaryKey.length > 0;
  const {
    unit
  } = schema;
  const cols = (0, _types.memoryColumns)(schema);
  const rows = new Map();
  ...
  const meta = new Map();
  const pkOf = row => (0, _args_key.cacheKeyOf)(schema.primaryKey.map(column => String(row[column])));
  ...
  function matching(where) {
    const out = [];
    for (const entry of candidates(where)) if ((0, _query.matchesWhere)(entry[1], where)) out.push(entry);
    return out;
  }
  ...
  function overwriteWith(where, incoming) {
    if (__DEV__) (0, _query.assertRowsMatchWhere)(schema.table, where, incoming);
    const before = matching(where).map(entry => entry[1]);
    const changed = (0, _unit_diff.changedUnits)(before, incoming, unit, cols);
    if (changed.size) {
  ...
  return {
    ...
    async upsert(incoming) {
      ...
      for (const row of incoming) {
        ...
        if (held && (0, _unit_diff.rowSignature)(held, cols) === (0, _unit_diff.rowSignature)(row, cols)) continue;
    ...
    getOne(where) {
      ...
      for (const [, row] of candidates(where)) if ((0, _query.matchesWhere)(row, where)) return row;
      return undefined;
    },
    find(where, opts) {
      (0, _read_coverage.noteTableRead)();
    ...
    findIn(where, column, values, _opts) {
        ...
        for (const value of wanted) {
          for (const spelling of spellingsOf(value)) {
            for (const [key, row] of candidates({
              ...
              if (emitted.has(key) || !(0, _query.matchesWhere)(row, where) || !inValues(wanted, row[column])) continue;
              emitted.add(key);
              out.push(row);
            }
          }
      ...
      for (const [, row] of candidates(where)) {
        if ((0, _query.matchesWhere)(row, where) && inValues(wanted, row[column])) out.push(row);
      }
      return out;
    },
    has(where) {
    ...
    unitsWhere(where) {
      (0, _read_coverage.noteTableRead)();
      const seen = new Set();
      for (const [, row] of candidates(where)) if ((0, _query.matchesWhere)(row, where)) seen.add(String(row[unit]));
      return [...seen];
    },
    getMeta(where) {
