...
function createSqliteRowTable(schema, conn, nativeShredSpec) {
  ...
  async function withDeferredIndexes(write) {
    const defer = secondaryIndexes.length > 0 && (deferrals > 0 || (0, _connection.readRows)(conn, `SELECT 1 FROM ${schema.table} LIMIT 1;`).length === 0);
  ...
  return {
    init() {
      const tableExists = (0, _connection.readRows)(conn, `PRAGMA table_info(${schema.table});`).length > 0;
      const plan = (0, _schema.planSchemaMigration)(schema, {
        tableExists,
        stamp: (0, _schema.readUserVersion)(conn)
      }, nativeShredSpec);
      if (plan === 'rebuild') {
