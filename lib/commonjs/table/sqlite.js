...
var _unit_diff_sql = require("./unit_diff_sql.js");
 ...
 */
function createSqliteRowTable(schema, conn, nativeShredSpec, options = {}) {
  ...
  const asyncStage = (0, _unit_diff_sql.stageNames)(schema.table, stageFingerprint, 'async');
  const asyncDiff = (0, _unit_diff_sql.unitDiffSql)(schema, asyncStage, MAX_BIND_VARIABLES);
  const syncDiff = (0, _unit_diff_sql.unitDiffSql)(schema, (0, _unit_diff_sql.stageNames)(schema.table, stageFingerprint, 'sync'), MAX_BIND_VARIABLES);
  ...
  const unitsOf = rows => rows.length ? new Set(rows.map(row => String(row[schema.unit]))) : _change_set.NO_CHANGES;
  ...
  const unitsLanded = (where, rows) => {
    ...
    return {
      changes: units.size ? units : _change_set.NO_CHANGES,
      rows
    };
  };
  ...
  return {
    ...
    unitsWhere(where) {
      ...
      return (0, _connection.readRows)(conn, `SELECT DISTINCT ${schema.unit} AS unit FROM ${schema.table}${sql};`, params).map(row => String(row.unit));
    },
    getMeta(where) {
