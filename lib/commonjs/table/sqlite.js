...
function createSqliteRowTable(schema, conn, nativeShredSpec) {
  ...
  const digestExpr = (0, _query.digestColumns)(schema).map(column => `ifnull(${column},'')`).join(" || char(1) || ");
  function readDigests(where, column, values) {
    const out = new Map();
    const filter = (0, _query.whereClause)(where);
    const select = `SELECT ${column} AS id, ${digestExpr} AS digest FROM ${schema.table}`;
    if (!values) {
      for (const row of (0, _connection.readRows)(conn, `${select}${filter.sql};`, filter.params)) {
    ...
    for (const chunk of (0, _collections.chunkList)(values, DEFAULT_IN_CHUNK)) {
      const sql = `${select}${prefix}${column} IN (${bindList(chunk.length)});`;
      for (const row of (0, _connection.readRows)(conn, sql, [...filter.params, ...chunk])) {
        out.set(String(row.id), String(row.digest));
      }
    }
    return out;
