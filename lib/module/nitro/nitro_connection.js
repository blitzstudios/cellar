  ...
  if (opts?.dedicatedReader) {
    try {
      ...
      reportStoreDegradation({
        ...
        context: 'failed to open the dedicated reader handle — reads fall back to the writer, where a read that needs a transaction can collide ' + 'with an ingest and degrade the store onto its in-memory backend',
        error,
        extra: {
          connection: name
        }
...
export function bindSqliteBackend(label, bind) {
...
export function bindSqliteStore(label, dbName, setBackend, createSqliteBackend, opts) {
