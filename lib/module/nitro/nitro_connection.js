...
export function bindSqliteStore(label, dbName, store, opts) {
  ...
  reportStoreDegradation({
    ...
    context: 'failed to bind SQLite, and again after deleting the database — the store runs on an in-memory table until a retry binds it',
    error: firstError,
    extra: {
      label,
      afterDeleting: String(secondError?.message ?? secondError)
    }
  });
}
