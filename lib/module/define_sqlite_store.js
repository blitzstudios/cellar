...
export function defineSqliteStore(config) {
  ...
  const buildMemoryBackend = () => config.buildBackend(createMemoryRowTable(config.schema), version, NO_CAPS);
  ...
  return {
    ...
    createSqliteBackend: conn => {
      ...
      const guarded = guardedConnection(conn, (error, op) => degrade({
      ...
      }), (error, op) => reportStoreDegradation({
      ...
      const backend = config.buildBackend(createSqliteRowTable(config.schema, guarded, config.nativeShredSpec), version, config.sqliteCapabilities?.(guarded) ?? NO_CAPS);
