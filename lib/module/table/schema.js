...
export function schemaFingerprint(schema, nativeShredSpec) {
  ...
  const canonical = cacheKey(`r${schema.rebuildVersion ?? 0}`, schema.table, cols.join('|'), `pk(${schema.primaryKey.join(',')})`, indexes.join('|'), meta, shredFingerprint(nativeShredSpec));
