...
export function schemaFingerprint(schema, nativeShredSpec) {
  const canonical = cacheKey(`r${schema.rebuildVersion ?? 0}`, schema.table, columnsCanonical(schema), ...structureCanonical(schema, nativeShredSpec));
...
export function schemaStructureStamp(schema, nativeShredSpec) {
  const canonical = cacheKey(`r${schema.rebuildVersion ?? 0}`, schema.table, ...structureCanonical(schema, nativeShredSpec));
