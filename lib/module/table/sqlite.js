...
import { assertRowsMatchWhere, comparator, digestColumns, whereClause } from "./query.js";
...
export function createSqliteRowTable(schema, conn, nativeShredSpec) {
  ...
  const digestExpr = digestColumns(schema).map(column => `ifnull(${column},'')`).join(" || char(1) || ");
  function readDigests(where, column, values) {
    ...
    if (!values) {
      for (const row of readRows(conn, `${select}${filter.sql};`, filter.params)) {
    ...
    for (const chunk of chunkList(values, DEFAULT_IN_CHUNK)) {
      ...
      for (const row of readRows(conn, sql, [...filter.params, ...chunk])) {
        out.set(String(row.id), String(row.digest));
      }
    }
    return out;
