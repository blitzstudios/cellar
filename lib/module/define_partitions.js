...
import { partitionLabel, cacheKeyOf } from "./args_key.js";
...
import { createDerivedValues, derivedValueMemo } from "./read/derived_values.js";
...
export function definePartitions(config) {
  ...
  const bump = (key, changes = ALL_UNITS) => {
  ...
  return {
    ...
    derive: () => def => {
      const bound = createMemos(name, memoBinding, {
        [def.name]: derivedValueMemo(def.max)
      });
      return createDerivedValues({
        store: name,
        table,
        filter: where,
        memo: bound[def.name],
