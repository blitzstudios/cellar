...
import { cacheKey, partitionLabel } from "./args_key.js";
import { createFetchIngest } from "./write/fetch_ingest.js";
import { createReadSurface, useResult } from "./read/surface.js";
import { createBoundedLru, createMemos } from "./caches.js";
import { addressesPartition, NO_PARTS } from "./reactivity/version_atom.js";
import { partitionKeyOf } from "./read/partition_fields.js";
import { createRowProjection, rowVmMemo } from "./read/projection.js";
import { NO_PRIMING } from "./prime_state.js";
import { offHeapStatus } from "./store_result.js";
 ...
 */
export function definePartitions(config) {
  const {
    name,
    table,
    version,
  ...
  const interned = toId ? createBoundedLru(config.internMax ?? INTERN_MAX) : undefined;
  ...
  const fetchedAt = createBoundedLru(config.internMax ?? INTERN_MAX);
    ...
    if (spec.canShredNatively?.(partition) === false) {
      ...
      try {
        ...
        reportStoreDegradation({
          ...
          extra: {
            store: name,
            partition: partitionLabel(toParts(key))
          }
        });
        count = inJs();
      }
  ...
  const ingest = fetchSpec ? createFetchIngest({
  ...
  function usePrimeAndVersion(args, options) {
    ...
    const status = runSubscribed(() => offHeapStatus(isEnabled, isEnabled && surface.has(key), prime));
  ...
  function readManyOf() {
    return def => {
      ...
      const publish = surface.readMany();
      return publish({
        ...def,
  ...
  function readGroupedOf() {
    return def => {
      ...
      const publish = surface.readGrouped();
      return publish({
        ...def,
        groups
      });
  ...
  return {
    ...
    memos: decls => createMemos(name, {
      parts: toParts,
      version: versionOf
    }, decls),
    project: () => def => {
      const bound = createMemos(name, {
        parts: toParts,
        version: versionOf
      }, {
        [def.name]: rowVmMemo(def.max)
      });
      return createRowProjection({
        store: name,
        table,
        filter: where,
        memo: bound[def.name]
