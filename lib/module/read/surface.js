...
export function createReadSurface(kernel) {
  ...
  function defineRead(def) {
    ...
    const gatesFor = (args, parts, vary, wanted) => readGates(def, args, wanted && addressesPartition(parts), vary);
  ...
  function manyRead(def, resolve, select, noneNamed) {
    ...
    const gatesFor = (args, partitions, vary, wanted) => readGates(def, args, wanted && partitions.some(addressesPartition), vary);
