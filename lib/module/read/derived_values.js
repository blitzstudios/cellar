...
export function createDerivedValues(ctx, def) {
  ...
  const warnOnThrash = count => {
    ...
    console.warn(`[${store}_store] the '${def.name}' derived values were asked for ${count} units but hold ${def.max}, so this read ` + 'evicts what it just built and rebuilds every value on every change. Raise `max` past the largest read, ' + `or read a narrower slice.${def.advice ? ` ${def.advice}` : ''}`);
  };
